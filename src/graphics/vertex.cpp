...
void AddVertexArray(prime::graphics::VertexArray &vtArray) {
  auto vtBuffers = vtArray.buffers();
  ...
  vtArray.mutate_index(vaoIndex);

  for (auto b : *vtBuffers) {
    ...
    if (auto attrs = b->attributes()) {
      for (auto a : *attrs) {
        glVertexAttribPointer(uint32(a->slot()), a->size(), a->type(),
                              a->normalized(), b->stride(),
  ...
  static inline const bool data =
      prime::common::AddResourceHandle<prime::graphics::VertexArray>({
          .Process =
              [](ResourceData &data) {
                auto hdr =
                    utils::GetFlatbuffer<prime::graphics::VertexArray>(data);
                AddVertexArray(*hdr);
              },
          .Delete =
              [](ResourceData &data) {
          ...
          .Handle = [](ResourceData &data) -> void * {
            return utils::GetFlatbuffer<prime::graphics::VertexArray>(data);
          },
      });
};
